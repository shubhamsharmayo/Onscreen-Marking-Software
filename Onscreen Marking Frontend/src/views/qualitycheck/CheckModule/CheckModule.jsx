...
import CheckBooklet from "components/QuestionSection/CheckBooklet";
...
const CheckModule = () => {
  ...
  const [pageQualityData, setPageQualityData] = useState([]);
  ...
  const handlePageQuality = (status) => {
    const currentPage = answerImageDetails.find(
      (img) => img._id === evaluatorState.currentAnswerPdfImageId
    );

    if (!currentPage) return;

    setPageQualityData((prev) => {
      const filtered = prev.filter((p) => p.pageId !== currentPage._id);
      ...
      <>
        ...
        <div className="flex h-screen w-full flex-row overflow-hidden">
          ...
          <div className="flex h-full w-[13%] flex-col bg-[#F3F4F6] sm:w-[20%] md:w-[14%] lg:w-[13%]">
            ...
            {/* ✅ ONLY ICONS SCROLL */}
            <div className="scrollbar-hide h-full flex-grow overflow-y-auto">
          ...
          >
            ...
            <div className="mb-2 flex justify-center gap-2">
              ...
              <button
                ...
                onClick={() => handlePageQuality("not_ok")}
              >
                ❌ Page Not OK
              </button>
            </div>
          ...
          <div className="scrollbar-hide h-full overflow-y-auto sm:w-[25%] md:w-[17%] lg:block lg:w-[17%]">
            <CheckBooklet
              ...
              pageQualityData={pageQualityData}
              setPageQualityData={setPageQualityData}
              socket={socket}
              id={id}
              ...
              totalPages={answerImageDetails.length}
            />
          </div>
        </div>
