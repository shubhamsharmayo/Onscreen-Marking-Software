...
import ImageContainer from "components/Imagecontainer/ImageContainers";
...
const CheckModule = () => {
  ...
  useEffect(() => {
    ...
    const payload = {
      ...
      evaluatorId: taskDetails?.assignedBy,
  ...
  useEffect(() => {
    const getTaskDetails = async () => {
      try {
        ...
        dispatch(setCurrentBookletIndex(task.currentFileIndex - 1));
        ...
        return (
          <div
            ...
            onClick={() => {
              if (!isDisabled) {
                toast.warning(
                  `spend atleast ${schemaDetails?.perPage} seconds`
                );
                return;
              }
