...
export const submitTaskByType = async (
  ...
  try {
    ...
    if (type === "question" || type === "questions") {
      ...
      const taskDetailRes = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/tasks/get/task/${taskId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      const answerPdfDetails = taskDetailRes.data?.answerPdfDetails;

      // ✅ Check remark
      const hasRemark = !!answerPdfDetails?.remark;

      console.log("✅ Remark:", answerPdfDetails?.remark);
      console.log("✅ hasRemark:", hasRemark);
